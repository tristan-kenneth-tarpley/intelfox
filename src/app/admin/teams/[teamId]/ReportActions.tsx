...
const ReportActions = ({ team }: { team: Teams }) => {
  const [prepState, prepFormAction] = useFormState(
    handlePrepIntelReportCommand,
    { teamId: team.id },
  );
  const [generateState, generateFormAction] = useFormState(
    handleGenerateIntelReportCommand,
    { teamId: team.id },
  );
  const [teamSyncState, teamSyncFormAction] = useFormState(
    handleDailyTeamSyncSubmission,
    { teamId: team.id },
  );
  ...
  return (
    <VStack align="start">
      ...
      <ToastOnFormCompletion
        title={prepState.title ?? "Report prepped"}
        message={prepState.message}
      />
      <ToastOnFormCompletion
        title={"Daily sync has started"}
        message={teamSyncState.message}
      />
